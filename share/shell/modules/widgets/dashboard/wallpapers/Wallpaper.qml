...
PanelWindow {
    ...
    property string mpvShaderDir: Paths.cachePath("mpv_shaders_" + (currentScreenName ? currentScreenName : "ALL"))
    ...
    Binding {
        target: wallpaper
        property: "weWallpapers"
        value: GlobalStates.wallpaperManager.weWallpapers
        when: GlobalStates.wallpaperManager !== null && GlobalStates.wallpaperManager !== wallpaper
    }
    ...
    Binding {
        target: wallpaper
        property: "wePaths"
        value: GlobalStates.wallpaperManager.wePaths
        when: GlobalStates.wallpaperManager !== null && GlobalStates.wallpaperManager !== wallpaper
    }
    ...
    Binding {
        target: wallpaper
        property: "weTitles"
        value: GlobalStates.wallpaperManager.weTitles
        when: GlobalStates.wallpaperManager !== null && GlobalStates.wallpaperManager !== wallpaper
    }
    ...
    function getThumbnailPath(filePath) {
        ...
        var thumbnailPath = Paths.cachePath("thumbnails/" + relativeDir + "/" + thumbnailName);
    ...
    function getLockscreenFramePath(filePath) {
        ...
        if (fileType === 'video' || fileType === 'gif') {
            ...
            var cachePath = Paths.cachePath("lockscreen/" + fileName + ".jpg");
            return cachePath;
        }
    ...
    function runMatugenForCurrentWallpaper() {
        ...
        if (currentWallpaper && initialLoadCompleted) {
            ...
            var commandWithConfig = ["matugen", "image", matugenSource, "--source-color-index", "0", "-c", Paths.asset("matugen/config.toml"), "-t", wallpaperConfig.adapter.matugenScheme];
    ...
    Process {
        ...
        running: false
        command: ["test", "-f", Paths.cachePath("wallpapers.json")]
    ...
    component WallpaperImage: Item {
        ...
        onSourceChanged: {
            var prevType = previousSource !== "" ? getFileType(previousSource) : "";
            ...
            if (source) {
                ...
                if (fileType === 'image' || fileType === 'wallpaperengine') {
                    killMpvpaperProcess.running = true;
                }
                if (prevType === 'wallpaperengine' && fileType !== 'wallpaperengine') {
                    stopWeProcess.running = true;
                }
            }
        }
        ...
        Process {
            id: stopWeProcess
            running: false
            command: ["bash", wallpaper.weScriptPath, "stop", wallpaper.currentScreenName]
        }
